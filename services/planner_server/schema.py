...
ALLOWED_SKILLS = {"locate", "navigate", "pick", "return", "inspect", "fetch"}
...
def _intent_flags(command: str) -> Tuple[bool, bool, bool, bool]:
    text = command.lower()
    ...
    return wants_pick, wants_return, wants_inspect, wants_nav_only


def generate_stub_plan(user_command: str, world_state: Dict) -> Dict:
    target = _infer_object_name(user_command)
    wants_pick, wants_return, wants_inspect, wants_nav_only = _intent_flags(user_command)
