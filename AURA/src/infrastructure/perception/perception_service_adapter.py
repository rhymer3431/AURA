...
from src.domain.memory.short_term_graph_memory import ShortTermGraphMemory
...
from src.domain.perception.scene_graph_tensor_frame import SceneGraphTensorFrame
...
from src.utils.streaming.serializer import serialize_entity_record
class PerceptionServiceAdapter:
    """Adapter that wraps the YOLO + LTM perception system to satisfy PerceptionPort."""
    ...
    def __init__(
        self,
        yolo_weight: str = "models/yoloe-26s-seg.pt",
    ...
    def build_scene_graph_tensor_frame(
        self,
    ...
    def _build_relations_stub(self, nodes: List[EntityNode]) -> List[Tuple[int, int, int]]:
        ...
        for i in range(n):
            ...
            id_i = nodes[i].entity_id

            for j in range(i + 1, n):
                cj_x, cj_y, cj_w, cj_h = self._box_center_and_size(nodes[j].box)
                id_j = nodes[j].entity_id

                # --- left/right ---
                margin_x = 0.05 * (ci_w + cj_w)
                if ci_x + margin_x < cj_x:
                    relations.append((id_i, id_j, self.RELATION_TYPES["left_of"]))
                    relations.append((id_j, id_i, self.RELATION_TYPES["right_of"]))
                elif cj_x + margin_x < ci_x:
                    relations.append((id_j, id_i, self.RELATION_TYPES["left_of"]))
                    relations.append((id_i, id_j, self.RELATION_TYPES["right_of"]))

                # --- above/below ---
                margin_y = 0.05 * (ci_h + cj_h)
                if ci_y + margin_y < cj_y:
                    relations.append((id_i, id_j, self.RELATION_TYPES["above"]))
                    relations.append((id_j, id_i, self.RELATION_TYPES["below"]))
                elif cj_y + margin_y < ci_y:
                    relations.append((id_j, id_i, self.RELATION_TYPES["above"]))
                    relations.append((id_i, id_j, self.RELATION_TYPES["below"]))

                # --- overlaps (IoU) ---
                iou = self._box_iou(nodes[i].box, nodes[j].box)
                if iou > 0.3:
                    relations.append((id_i, id_j, self.RELATION_TYPES["overlaps"]))
                    relations.append((id_j, id_i, self.RELATION_TYPES["overlaps"]))
                ...
                avg_w = 0.5 * (ci_w + cj_w)
                if dist < 2.0 * avg_w:
                    relations.append((id_i, id_j, self.RELATION_TYPES["near"]))
                    relations.append((id_j, id_i, self.RELATION_TYPES["near"]))
    ...
    def process_frame(
        ...
        sg_diff = self.stm.push(sg_frame)
        ...
        return sg_frame, sg_diff, grin_outputs
