...
class YoloWorldTrackNodeEncoder:
    """
    - YOLO(models/yoloe-26s-seg.pt) 추론 결과에서 노드 로드
    - P3 hook으로 ROI feature 추출
    - 각 주요 YOLO 레이어에 forward hook을 걸어 모듈별 로그를 남김 (yw_#)
    """

    def __init__(self, weight: str = "models/yoloe-26s-seg.pt", device: Optional[str] = None, logger: Optional[PipelineLogger] = None):
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        self.logger = logger
        self._pin_mobileclip_weight()

        # 1) Always load via Ultralytics YOLO wrapper.
        self.model = YOLO(weight).to(self.device)
        self.model.eval()

        # 2) 텍스트 임베딩 캐시 (if supported by model)
        txt = getattr(self.model.model, "txt_feats", None)
        if txt is not None:
    ...
    def _register_layer_hooks(self):
        """
        Register forward hooks on key YOLO layers with semantic names (yw_#).
        """
    ...
    def _pin_mobileclip_weight(self) -> None:
        """Force Ultralytics to resolve MobileCLIP from local pre-downloaded weight."""
        try:
        ...
        candidates = [
            aura_root / "models" / "mobileclip2_b.ts",
            aura_root / "mobileclip2_b.ts",
            aura_root / "weights" / "mobileclip2_b.ts",
            aura_root.parent / "weights" / "mobileclip2_b.ts",
        ]
        clip_path = next((p for p in candidates if p.exists()), None)
        if clip_path is None:
            return

        clip_dir = str(clip_path.parent.resolve())
        current = str(SETTINGS.get("weights_dir", ""))
        if current != clip_dir:
            SETTINGS.update(weights_dir=clip_dir)
    ...
    def nodes_from_result(self, result):
        ...
        for b in result.boxes:
            ...
            if self.text_emb is not None and cls_id < self.text_emb.shape[0]:
                text_feat = self.text_emb[cls_id]  # (D_text,)
            else:
                text_feat = torch.empty((0,), device=self.device, dtype=roi_feat.dtype)
            node_feat = torch.cat([roi_feat, text_feat], dim=0)  # (C + D_text, or C)
