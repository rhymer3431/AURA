...
function buildMockState(apiBaseUrl: string, nowMs: number) {
  ...
  return {
    ...
    memory: {
      ...
      scratchpad: {
        taskState: "idle",
        instruction: "Start scripts/run_dashboard.ps1 for the full backend.",
    ...
    architecture: {
      ...
      mainControlServer: {
        ...
        status: "inactive",
        summary: "Ready",
        detail: "no active runtime session",
        required: true,
        metrics: {
          mode: "",
          taskState: "idle",
      ...
      modules: {
        ...
        memory: {
          name: "Memory",
          status: "inactive",
          summary: "No active memory task",
          detail: "scratchpad idle",
          required: true,
          metrics: {},
        },
        s2: {
...
export function buildDevApiFallbackResponse({
  ...
  return jsonResponse(
    {
      error: `AURA dashboard backend is not running at ${normalizedBase}. Start scripts/run_dashboard.ps1 or point VITE_AURA_API_BASE/AURA_DASHBOARD_PROXY_TARGET to a live backend.`,
      mock: true,
    },
    503,
  );
