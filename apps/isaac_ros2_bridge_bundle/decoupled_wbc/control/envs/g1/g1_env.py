...
class G1Env(HumanoidEnv):
    def __init__(
        ...
        **kwargs,
        ...
        if self.with_hands:
            self._hands = Hands()
            self._hands.left = G1ThreeFingerHand(is_left=True, config=self.config)
            self._hands.right = G1ThreeFingerHand(is_left=False, config=self.config)
    ...
    def set_ik_indicator(self, teleop_cmd):
        ...
        elif self.config["SIMULATOR"] == "isaacsim":
            return
        else:
