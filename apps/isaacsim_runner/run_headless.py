...
def _ensure_world_environment(stage_obj) -> None:
    ...
    root_g1 = []
    for prim in stage_obj.GetPseudoRoot().GetChildren():
        name = prim.GetName()
        if name.lower().startswith("g1"):
            root_g1.append(f"/{name}")

    for src_path in root_g1:
        dest_path = f"/World/Robots/{src_path.split('/')[-1]}"
        _move_prim(src_path, dest_path)
...
def _find_motion_root_prim(stage_obj, robot_prim_path: str) -> str:
    ...
    if robot_prim_path.endswith("/pelvis"):
        parent = robot_prim_path.rsplit("/", 1)[0]
        if stage_obj.GetPrimAtPath(parent).IsValid():
            return parent
    return robot_prim_path
...
def _set_robot_start_height(stage_obj, robot_prim_path: str, z: float) -> None:
    try:
        ...
        logging.warning("Could not import pxr UsdGeom to set robot start height: %s", exc)
        return
    ...
    if translate_op is None:
        translate_op = xform.AddTranslateOp()
        translate_op.Set((0.0, 0.0, z))
        logging.info("Added translate op and set robot start height: prim=%s z=%.2f", robot_prim_path, z)
        return

    current = translate_op.Get() or (0.0, 0.0, 0.0)
    try:
    ...
    logging.info("Set robot start height: prim=%s z=%.2f", robot_prim_path, z)
...
def _run_native_isaac(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        while simulation_app.is_running():
            ...
            base_pose = _read_base_pose(stage_obj, robot_prim_path) if robot_prim_path else None
            base_roll = None
            base_pitch = None
            base_yaw = None
            base_height = None
