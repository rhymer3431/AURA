...
PD_JOINT_ORDER = LEG_JOINTS + WAIST_JOINTS + ARM_JOINTS
...
PD_KP_BY_NAME = dict(zip(PD_JOINT_ORDER, PD_KP))
PD_KD_BY_NAME = dict(zip(PD_JOINT_ORDER, PD_KD))


def log_robot_dof_snapshot(robot) -> Dict[str, Any]:
    try:
...
def apply_pd_gains(robot) -> Dict[str, Any]:
    ...
    for idx, name in enumerate(joint_names):
        kp = PD_KP_BY_NAME.get(name)
        kd = PD_KD_BY_NAME.get(name)
    ...
    try:
        ...
        legs = _log_gain_summary("legs", LEG_JOINTS, joint_names, applied_kps, applied_kds)
        waist = _log_gain_summary("waist", WAIST_JOINTS, joint_names, applied_kps, applied_kds)
        arms = _log_gain_summary("arms", ARM_JOINTS, joint_names, applied_kps, applied_kds)
