...
from decoupled_wbc.control.envs.g1.utils.isaac_ros_interface import (
    IsaacBodyCommandSender,
    IsaacBodyStateProcessor,
)
...
class G1Body(Env):
    def __init__(self, config: Dict[str, Any]):
        ...
        if config.get("SIMULATOR", "") == "isaacsim":
            self.body_state_processor = IsaacBodyStateProcessor(config=config)
            self.body_command_sender = IsaacBodyCommandSender(config=config)
        else:
