...
from decoupled_wbc.control.envs.g1.utils.isaac_ros_interface import (
    IsaacHandCommandSender,
    IsaacHandStateProcessor,
)
from decoupled_wbc.control.envs.g1.utils.state_processor import HandStateProcessor


class G1ThreeFingerHand(Env):
    def __init__(self, is_left: bool = True, config: Dict[str, Any] = None):
        super().__init__()
        self.is_left = is_left
        config = config or {}
        if config.get("SIMULATOR", "") == "isaacsim":
            self.hand_state_processor = IsaacHandStateProcessor(config=config, is_left=self.is_left)
            self.hand_command_sender = IsaacHandCommandSender(config=config, is_left=self.is_left)
        else:
