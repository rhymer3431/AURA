...
def init_channel(config: Dict[str, Any]) -> None:
    ...
    if config.get("SIMULATOR", "") == "isaacsim":
        # Isaac Sim integration uses ROS 2 topics and does not require Unitree channels.
        return
...
class SimulatorFactory:
    ...
    def create_simulator(config: Dict[str, Any], env_name: str = "default", **kwargs):
        ...
        if simulator_type == "mujoco":
            ...
            return SimulatorFactory._create_isaacsim_simulator(config, env_name, **kwargs)
        else:
            print(
    ...
    @staticmethod
    def _create_isaacsim_simulator(config: Dict[str, Any], env_name: str = "default", **kwargs):
        """Isaac Sim is driven externally via ROS 2 bridge and adapter nodes."""
        return None
