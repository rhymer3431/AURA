...
"@
    try {
        ...
        $Process = Start-Process -FilePath $PythonPath -ArgumentList @($TempScript) -Wait -PassThru -NoNewWindow
...
$PythonExe = if ($env:AURA_DASHBOARD_PYTHON_EXE) { $env:AURA_DASHBOARD_PYTHON_EXE } elseif (Test-Path $DefaultPythonExe) { $DefaultPythonExe } else { "python" }
...
$CargoCmd = if ($env:AURA_DASHBOARD_CARGO_CMD) { $env:AURA_DASHBOARD_CARGO_CMD } elseif (Get-Command "cargo.exe" -ErrorAction SilentlyContinue) { "cargo.exe" } elseif (Test-Path $DefaultCargoPath) { $DefaultCargoPath } else { "cargo.exe" }
...
$CargoCommandInfo = if ($CargoCmdIsPath) { $null } else { Get-Command $CargoCmd -ErrorAction SilentlyContinue }
if (-not $CargoCmdIsPath -and -not $CargoCommandInfo) {
    throw "Rust cargo executable not found: $CargoCmd"
}
...
$BackendReadyTimeoutSec = if ($env:AURA_DASHBOARD_BACKEND_READY_TIMEOUT_SEC) { [int]$env:AURA_DASHBOARD_BACKEND_READY_TIMEOUT_SEC } else { 60 }
