...
def _patch_eagle_processor_module_compat() -> None:
    ...
    if not hasattr(image_cls, "_prepare_input_images"):
        def _prepare_input_images(self, images, do_convert_rgb=None, input_data_format=None, device=None):
            # Try known method names across different transformers versions.
            kwargs = {}
            if do_convert_rgb is not None:
                kwargs["do_convert_rgb"] = do_convert_rgb
            ...
            for method_name in (
                ...
                if hasattr(self, method_name):
                    LOGGER.info("Eagle processor: routing _prepare_input_images -> %s", method_name)
                    method = getattr(self, method_name)
                    try:
                        return method(images=images, **kwargs)
                    except TypeError:
                        return method(images, **kwargs)

            # Final fallback: call preprocess directly and extract pixel_values.
            LOGGER.warning(
                "Eagle processor: no known prepare method found, falling back to preprocess()"
            )
            preprocess_kwargs = {}
            if do_convert_rgb is not None:
                preprocess_kwargs["do_convert_rgb"] = do_convert_rgb

            if getattr(self, "_groot_prepare_input_in_progress", False):
                # Avoid infinite recursion on unexpected processor variants.
                return images

            setattr(self, "_groot_prepare_input_in_progress", True)
            try:
                return self.preprocess(images, return_tensors="pt", **preprocess_kwargs)
            except Exception as exc:
                LOGGER.warning(
                    "Eagle processor: preprocess() fallback failed (%s). Returning raw images.",
                    exc,
                )
                return images
            finally:
                setattr(self, "_groot_prepare_input_in_progress", False)

        image_cls._prepare_input_images = _prepare_input_images  # type: ignore[attr-defined]
        LOGGER.info("Eagle processor: _prepare_input_images compatibility shim installed")
