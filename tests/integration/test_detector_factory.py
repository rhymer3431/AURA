...
def test_detector_factory_falls_back_cleanly_when_model_path_is_missing() -> None:
    backend = create_detector_backend(DetectorFactoryConfig(model_path="artifacts/models/__missing__.engine", fallback_label="apple"))
...
def test_detector_factory_returns_structured_runtime_report() -> None:
    selection = select_detector_backend(DetectorFactoryConfig(model_path="artifacts/models/__missing__.engine", fallback_label="apple"))

    assert selection.report.backend_name == "ultralytics_yolo"
    assert selection.report.engine_exists is False
...
def test_detector_factory_uses_explicit_model_path_and_falls_back_cleanly(tmp_path: Path) -> None:
