...
set "PLAY_SCRIPT=%REPO_DIR%\play_g1_internvla_navdp.py"
...
set "NAVDP_SERVER_LAUNCHER=%REPO_DIR%\run_navdp_server_windows.bat"
set "INTERNVLA_SERVER_LAUNCHER=%REPO_DIR%\run_internvla_nav_server_windows.bat"
...
if not exist "%PLAY_SCRIPT%" (
    echo [ERROR] InternVLA/NavDP play script not found: %PLAY_SCRIPT%
    exit /b 1
)
...
if not "%NAV_COMMAND_API_PORT%"=="0" (
    ...
    echo [INFO] Command helper : %REPO_DIR%\send_internvla_nav_command_windows.bat
)
...
call "%ISAACSIM_PYTHON%" "%PLAY_SCRIPT%" ^
    --policy "%POLICY%" ^
