"""Canonical navigation runtime that owns the frame loop for AURA."""
...
class NavigationRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self._observation_module = ObservationModule(self.planning_session)
        self._planning_module = PlanningModule(self.planning_session)
        self._execution_module = ExecutionModule(self._executor)
        self._world_model_module: WorldModelModule | None = None
        self._mission_module: MissionModule | None = None
        self._runtime_io_module = RuntimeIOModule(
            open_bridge=self._ensure_runtime_bridge,
            publish_detector_capability=self._publish_detector_capability,
            publish_runtime_snapshot=self._publish_runtime_snapshot,
        )
    ...
    @property
    def observation_module(self) -> ObservationModule:
        return self._observation_module

    @property
    def planning_module(self) -> PlanningModule:
        return self._planning_module

    @property
    def execution_module(self) -> ExecutionModule:
        return self._execution_module

    @property
    def world_model_module(self) -> WorldModelModule:
        module = getattr(self, "_world_model_module", None)
        if module is None:
            supervisor = self.supervisor
            if hasattr(supervisor, "update_world_model"):
                module = WorldModelModule(supervisor)
            else:
                module = SimpleNamespace(
                    supervisor=supervisor,
                    memory_write=MemoryWritePath(supervisor.memory_service),
                    memory_read=MemoryReadPath(supervisor.memory_service),
                )
            self._world_model_module = module
        return module

    @property
    def mission_module(self) -> MissionModule:
        module = getattr(self, "_mission_module", None)
        if module is None:
            self._mission_module = MissionModule(self.supervisor)
            module = self._mission_module
        return module

    @property
    def runtime_io_module(self) -> RuntimeIOModule:
        return self._runtime_io_module

    def initialize(self, simulation_app, stage, controller) -> None:
        self._controller = controller
        for _ in range(max(int(self.args.startup_updates), 0)):
            simulation_app.update()
        self.execution_module.initialize(simulation_app, stage)
        self.runtime_io_module.open()
        self._bootstrap_mode()
        self.runtime_io_module.publish_detector_capability()
    ...
    def update(self, frame_idx: int) -> None:
        if self._controller is None:
            raise RuntimeError("NavigationRuntime.initialize() must be called before update().")
        ...
        observation = self.observation_module.capture(frame_idx)
        if observation is not None:
            ...
            enriched = self.world_model_module.update(batch)
            self.mission_module.update(enriched, publish=self._viewer_publish)
            ...
            if active_memory_instruction != "":
                memory_context = self.world_model_module.memory_read.build_memory_context(
        ...
        robot_lin_vel_world = np.asarray(
            getattr(base_state, "lin_vel_w", np.zeros(3, dtype=np.float32)),
            dtype=np.float32,
        )
        robot_ang_vel_world = np.asarray(
            getattr(base_state, "ang_vel_w", np.zeros(3, dtype=np.float32)),
            dtype=np.float32,
        )
        ...
        self.runtime_io_module.publish(frame_idx=frame_idx, update=update, evaluation=evaluation)
    ...
    def _bootstrap_mode(self) -> None:
        ...
        if self._mode == "dual":
            ...
            if instruction != "":
                ...
                self.world_model_module.memory_write.set_planner_task(
    ...
    def _resolve_action_command(self, *, robot_pose: tuple[float, float, float]) -> ActionCommand | None:
        ...
        command = self.mission_module.propose_action(
            now=time.time(),
    ...
    def _ensure_runtime_bridge(self) -> None:
        ...
        self._world_model_module = WorldModelModule(self._supervisor)
        self._mission_module = MissionModule(self._supervisor)
        print(
    ...
    def _cancel_interactive_task(self, *, source: str) -> bool:
        ...
        if cancelled:
            self.world_model_module.memory_write.clear_planner_task(
...
def main() -> int:
    ...
    try:
        return run_g1_play(args, simulation_app, command_source=NavigationRuntime(args))
    except Exception as exc:  # noqa: BLE001
