...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Internal frame bridge launcher with IPC wiring.", parents=[BOOTSTRAP_PARSER])
...
def run_lightweight_frame_bridge(
    ...
    *,
    ...
    try:
        ...
        bridge.publish_health(HealthPing(component="frame_bridge", details={"bus": args.bus, "mode": "lightweight"}))
        if initial_notice != "":
        ...
        if args.frame_source == "live" and source_report.status != "ready":
            print(f"[FRAME_BRIDGE] live frame source unavailable: {source_report.notice}")
            return 1
        bridge.publish_notice(
            RuntimeNotice(
                component="frame_bridge",
...
def run_live_frame_bridge_app(args: argparse.Namespace) -> int:
    runtime_io = build_runtime_io(
        bus_kind=args.bus,
        endpoint=args.endpoint,
        bind=not bool(args.connect),
    ...
    try:
        return run_live_frame_bridge(args, bus=runtime_io.bus, shm_ring=runtime_io.shm_ring)
    finally:
        runtime_io.close(unlink_shm=bool((not args.connect) and args.bus == "zmq"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if str(args.frame_source).lower() == "synthetic":
        return run_lightweight_frame_bridge(args)
