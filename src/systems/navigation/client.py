...
def _image_bytes(rgb: np.ndarray, depth: np.ndarray) -> tuple[bytes, bytes]:
    ...
    Image.fromarray(_normalize_rgb(rgb)).save(rgb_buffer, format="JPEG", quality=95)

    depth_buffer = io.BytesIO()
    Image.fromarray(_normalize_depth(depth)).save(depth_buffer, format="PNG")
    return rgb_buffer.getvalue(), depth_buffer.getvalue()


def _history_file(rgb_history: np.ndarray | None) -> tuple[str, bytes, str] | None:
    if rgb_history is None:
        return None
    return ("history.npz", encode_rgb_history_npz(rgb_history), "application/octet-stream")
...
class NavDpClient:
    ...
    def step_pointgoal(
        self,
        point_goal_xy: np.ndarray,
        rgb: np.ndarray,
        depth: np.ndarray,
        *,
        rgb_history: np.ndarray | None = None,
    ) -> NavDpPlan:
        if self._active_backend == "heuristic":
        ...
        try:
            ...
            history_file = _history_file(rgb_history)
            if history_file is not None:
                files["history_npz"] = history_file
            payload = self._post(
                "/pointgoal_step",
                files=files,
                data={
