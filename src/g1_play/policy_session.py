...
class _TensorRtPolicySession:
    ...
    def __init__(self, policy_path: str, device_preference: str) -> None:
        ...
        print(f"[INFO] TensorRT session init: loading engine {policy_path}")
        ...
        print("[INFO] TensorRT session init: engine deserialized")
        self._context = self._engine.create_execution_context()
        if self._context is None:
            raise RuntimeError(f"Failed to create TensorRT execution context: {policy_path}")
        print("[INFO] TensorRT session init: execution context created")
        ...
        print(
            "[INFO] TensorRT session init: "
            f"input={self.input_name}{self.input_shape}/{self._input_dtype} "
            f"output={self.output_name}{self.output_shape}/{self._output_dtype}"
        )
        ...
        print("[INFO] TensorRT session init: CUDA buffers allocated and bound")
